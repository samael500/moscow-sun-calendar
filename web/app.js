...
(async()=>{
 ...
 try {
  ...
  const labels=new Set(['moscow','saint-petersburg','novosibirsk','yekaterinburg','krasnoyarsk','irkutsk','yakutsk','vladivostok','magadan','murmansk','petropavlovsk-kamchatsky']);
  for(const c of cities){
   const b=document.createElement('button');b.type='button';b.textContent=c.name;b.dataset.city=c.id;b.dataset.search=[c.name,c.region||''].join(' ');b.onclick=()=>select(c.id);list.append(b);
  ...
  document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{const view=({all:'0 0 1100 420',west:'0 70 365 350',east:'345 0 755 420',crimea:'88 365 42 25',belarus:'26 265 68 53'})[b.dataset.view];svg.setAttribute('viewBox',view);const scale=Math.sqrt(Number(view.split(' ')[2])/1100);svg.querySelectorAll('.dot').forEach(e=>e.setAttribute('r',4.5*scale));svg.querySelectorAll('.hit').forEach(e=>e.setAttribute('r',10*scale));svg.querySelectorAll('.map-label').forEach(e=>{e.style.fontSize=(12*scale)+'px';});document.querySelectorAll('[data-view]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));});
