...
Подготовленные GeoJSON хранятся локально. `scripts/build_map.py` проецирует сушу, гидрографию и точки одинаково; SHA256 каждого локального слоя сохраняется в `map-projection.json`. Геометрии уже разделены Natural Earth у 180°; перед обрезкой их копии переносятся на ±360°, затем каждая часть проецируется отдельно. Границы государств и дорог не добавлены.
